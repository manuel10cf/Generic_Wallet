...
<html lang="en">
<head>
    <title>Code coverage report for blocktrail-sdk-nodejs/lib/size_estimation.js</title>
    <meta charset="utf-8" />
...
<div class='wrapper'>
  <div class='pad1'>
    <h1>
      <a href="../../index.html">All files</a> / <a href="index.html">blocktrail-sdk-nodejs/lib</a> size_estimation.js
    </h1>
    <div class='clearfix'>
      <div class='fl pad1y space-right2'>
        <span class="strong">86.33% </span>
        <span class="quiet">Statements</span>
        <span class='fraction'>120/139</span>
      </div>
      <div class='fl pad1y space-right2'>
...
279
280
281</td><td class="line-coverage quiet"><span class="cline-any cline-yes">1x</span>
...
SizeEstimation.getLengthForScriptPush = function(length) {
    if (length &lt; 75) {
        ...
        return 3;
    } else <span class="missing-if-branch" title="else path not taken" >E</span>if (length &lt;= 0xffffffff) {
        return 5;
    } else {
<span class="cstat-no" title="statement not covered" >        throw new Error("Size of pushdata too large");</span>
    }
};
&nbsp;
...
&nbsp;
    ...
    if (length &lt; 65535) {
        ...
        numBytes = 4;
    } else <span class="missing-if-branch" title="else path not taken" >E</span>if (length &lt; 18446744073709551615) {
        numBytes = 8;
    } else {
<span class="cstat-no" title="statement not covered" >        throw new Error("Size of varint too large");</span>
    }
&nbsp;
    return 1 + numBytes;
};
&nbsp;
SizeEstimation.estimateMultisigStackSize = <span class="fstat-no" title="function not covered" >fu</span>nction(m, keys) {
    // Initialize with OP_0
    var stackSizes = <span class="cstat-no" title="statement not covered" >[0];</span>
    var i;
<span class="cstat-no" title="statement not covered" >    for (i = 0; i &lt; m; i++) {</span>
<span class="cstat-no" title="statement not covered" >        stackSizes.push(SizeEstimation.SIZE_DER_SIGNATURE);</span>
    }
&nbsp;
    var scriptSize = <span class="cstat-no" title="statement not covered" >1;</span> // OP_$m
<span class="cstat-no" title="statement not covered" >    for (i = 0; i &lt; keys.length; i++) {</span>
<span class="cstat-no" title="statement not covered" >        scriptSize += this.getLengthForScriptPush(keys[i].length) + keys[i].length;</span>
    }
<span class="cstat-no" title="statement not covered" >    scriptSize += 2; </span>// OP_$n OP_CHECKMULTISIG
<span class="cstat-no" title="statement not covered" >    return [stackSizes, scriptSize];</span>
};
&nbsp;
&nbsp;
...
&nbsp;
    ...
    if (isWitness) {
        ...
        <span class="missing-if-branch" title="else path not taken" >E</span>if (ws instanceof Buffer) {
...
&nbsp;
    ...
    } else <span class="missing-if-branch" title="else path not taken" >E</span>if (bitcoin.script.pubKeyHash.output.check(script)) {
        stackSizes = this.estimateP2PKHStackSize(compressed)[0];
    } else {
<span class="cstat-no" title="statement not covered" >        throw new Error("Unsupported script type");</span>
    }
&nbsp;
...
&nbsp;
    ...
    if (bitcoin.script.scriptHash.output.check(signScript)) {
        <span class="missing-if-branch" title="if path not taken" >I</span>if (null === rs) {
<span class="cstat-no" title="statement not covered" >            throw new Error("Cant estimate, missing redeem script");</span>
        }
        signScript = rs;
    }
&nbsp;
    <span class="missing-if-branch" title="if path not taken" >I</span>if (bitcoin.script.witnessPubKeyHash.output.check(signScript)) {
        var p2wpkh = <span class="cstat-no" title="statement not covered" >bitcoin.script.witnessPubKeyHash.output.decode(signScript);</span>
<span class="cstat-no" title="statement not covered" >        signScript = bitcoin.script.pubKeyHash.encode(p2wpkh);</span>
<span class="cstat-no" title="statement not covered" >        witness = true;</span>
    } else if (bitcoin.script.witnessScriptHash.output.check(signScript)) {
        <span class="missing-if-branch" title="if path not taken" >I</span>if (null === ws) {
<span class="cstat-no" title="statement not covered" >            throw new Error("Can't estimate, missing witness script");</span>
        }
        signScript = ws;
        witness = true;
    }
&nbsp;
    ...
    <span class="missing-if-branch" title="if path not taken" >I</span>if (allowedTypes.indexOf(type) === -1) {
<span class="cstat-no" title="statement not covered" >        throw new Error("Unsupported script type");</span>
    }
&nbsp;
...
SizeEstimation.estimateInputsSize = function(utxos, withWitness) {
    ...
    utxos.map(function(utxo) {
        ...
        <span class="missing-if-branch" title="if path not taken" >I</span>if (withWitness) {
<span class="cstat-no" title="statement not covered" >            witnessSize += estimate.witness;</span>
        }
    });
&nbsp;
    <span class="missing-if-branch" title="if path not taken" >I</span>if (withWitness &amp;&amp; <span class="branch-1 cbranch-no" title="branch not covered" >witnessSize &gt; 0)</span> {
<span class="cstat-no" title="statement not covered" >        inputSize += 2 + witnessSize;</span>
    }
&nbsp;
    return inputSize;
};
